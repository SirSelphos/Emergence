...
export class Hex {
    ...
    neighbor(direction){
        return this.addHex(Hex.direction(direction));
    }
    diagonalNeighbor(direction){
        return this.addHex(Hex.diagonals[direction]);
    }
    len(){
        return (Math.abs(this.q) + Math.abs(this.r) + Math.abs(this.s)) / 2;
    }
    distance(b){
        return this.subtractHex(b).len();
    }
    round(){
...
Hex.directions = [new Hex(0, -1, 1), new Hex(1, -1, 0), new Hex(1, 0, -1), new Hex(0, 1, -1), new Hex(-1, 1, 0), new Hex(-1, 0, 1)];
...
export class Layout {
    ...
    hexToPixel(h){
        typeof h === 'string' ? h = JSON.parse(h) : h
    ...
    pixelToHex(p){
        typeof p === 'string' ? p = JSON.parse(p) : p
        var M = this.orientation;
        var size = this.size;
        var origin = this.origin;
...
class Tests {
    ...
    static testHexArithmetic(){
        Tests.equalHex("hex_add", new Hex(4, -10, 6), new Hex(1, -3, 2).addHex(new Hex(3, -7, 4)));
        Tests.equalHex("hex_subtract", new Hex(-2, 4, -2), new Hex(1, -3, 2).subtractHex(new Hex(3, -7, 4)));
    }
    static testHexDirection(){
        Tests.equalHex("hex_direction", new Hex(0, -1, 1), Hex.direction(0));
    }
    static testHexNeighbor(){
        Tests.equalHex("hex_neighbor", new Hex(1, -3, 2), new Hex(1, -2, 1).neighbor(0));
    }
